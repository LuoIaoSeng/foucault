...
export default async function SubmissionsPage({
  ...
  // Only admin or the course's educator can view submissions
          ...
          {assignments.map((assignment) => {
            ...
            return (
              <Surface key={assignment.id} className="rounded-2xl border border-(--tt-card-border-color) p-6">
                ...
                {subs.length === 0 ? (
                  <p className="text-sm text-(--tt-color-text-gray) py-4">
                    No submissions yet.
                  </p>
                ) : (
                  <div className="flex flex-col gap-2">
                    {subs.map((sub) => (
                      <div
                        key={sub.id}
                        className="flex items-center justify-between p-3 rounded-xl bg-(--tt-brand-color-50)"
                      >
                        <div className="flex items-center gap-3">
                          <div>
                        ...
                        <a
                          ...
                          className="flex items-center gap-1 text-sm text-(--tt-brand-color-500) hover:underline font-medium"
                        >
